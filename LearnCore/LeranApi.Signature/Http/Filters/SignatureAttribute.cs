...
using LeranApi.Signature.Redis;
using LeranApi.Signature.Security;
using LeranApi.Signature.Config;

namespace LeranApi.Signature.Http.Filters
{
    /// <summary>
    /// 签名认证
    /// </summary>
    public class SignatureAttribute : ActionFilterAttribute
    {
        public override void OnActionExecuting(System.Web.Http.Controllers.HttpActionContext actionContext)
        {
            ...
            if (!SignatureHelper.Verify(signData, secret, signatureHeader.Signature))
            {// 无效签名
                actionContext.Response = new HttpResponseMessage(HttpStatusCode.Unauthorized)
                {
                    ReasonPhrase = "无效签名"
